...
class Board:
	...
	def create_board_map_from_file(self, filename):
			...
			with open(path_to_file, "r") as f:
				for line in f:
					for col in range(self.WIDTH_BOARD):
						self.board_map[row][col] = self.create_object_from_key(line[col])
					row += 1
		...
		for it in range(3):
			randseed = sum(row.count('p') for row in self.board_map)
			path_counter = random.randint(0, randseed)
			...
			self.board_map[loot_pos // 15][(loot_pos % 15) - 1] = self.create_object_from_key("l")



	@staticmethod
	def create_object_from_key(key):
		#switch structure : create object corresponding to the type defined by the key parameter
		return key
